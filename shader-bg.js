/* ==========================================================================
   "Halftone" — animated WebGL1 pattern background
   Made with the 21st.dev Shader Builder. Fullscreen triangle, no libraries.
   Mounts on #shader-bg; pauses when the tab is hidden or the canvas is
...
(function () {
  ...
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    // Static fallback in the recipe's deepest colour.
  ...
  if (!gl) { canvas.style.background = '#0B1026'; return; }
  ...
  if (!vs || !fs) { canvas.style.background = '#0B1026'; return; }
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error('shader-bg:', gl.getProgramInfoLog(program));
    canvas.style.background = '#0B1026';
    return;
  }
  ...
  // #0B1026, #3D46E8, #B18CFF, #FFD6E7 — first 4 of 8 slots used.
  ...
  gl.uniform4f(uShape, 1.48, 0.88, 0.50, 0.00);
  gl.uniform4f(uSurface, 2.40, 0.91, 0.00, 1.00);
  gl.uniform4f(uFinish, 0.00, 0.00, 0.000, 0.06);
  gl.uniform4f(uTransform, 2926.0, 0.00, 0.00, 0.0);
  gl.uniform4f(uCursor, 0.0, 2.0, 0.65, 0.46); // presence 0 — cursor off

  var TIME_SCALE = 1.43; // speed 70/100
  ...
  function render(now) {
    ...
    var t = ((now || performance.now()) - start) / 1000 * TIME_SCALE;
    gl.uniform4f(uScene, width, height, t, 4.0);
    gl.uniform4f(uSpace, 0.0, 0.0, 0.0, 0.0); // no pan, pointer unused
  ...
  function play() {
    if (!frame && !document.hidden && onScreen) frame = requestAnimationFrame(render);
  }
  ...
  window.addEventListener('resize', function () {
    ...
    resize();
    if (!frame) { resize(); gl.drawArrays(gl.TRIANGLES, 0, 3); }
  });
